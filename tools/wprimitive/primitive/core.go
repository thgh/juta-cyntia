package primitive

import (
	"image"
	"math"
)

// PixelWeights holds per-pixel importance for scoring (length W*H).
// nil means uniform weight 1. Weights should be >= 1 typically.
var PixelWeights []float64

// WeightSum is the sum of all PixelWeights (or W*H if nil).
var WeightSum float64

// WeightCDF is a prefix-sum over PixelWeights for weighted position sampling.
var WeightCDF []float64

// WeightMax is the maximum pixel weight (for size scaling).
var WeightMax float64

// SetPixelWeights installs a weight map matching the target image size.
func SetPixelWeights(w, h int, weights []float64) {
	if weights == nil || len(weights) != w*h {
		PixelWeights = nil
		WeightCDF = nil
		WeightSum = float64(w * h)
		WeightMax = 1
		return
	}
	PixelWeights = weights
	WeightCDF = make([]float64, len(weights))
	var sum float64
	var maxW float64
	for i, v := range weights {
		if v < 0 {
			v = 0
			weights[i] = 0
		}
		sum += v
		WeightCDF[i] = sum
		if v > maxW {
			maxW = v
		}
	}
	if sum <= 0 {
		PixelWeights = nil
		WeightCDF = nil
		WeightSum = float64(w * h)
		WeightMax = 1
		return
	}
	WeightSum = sum
	WeightMax = maxW
}

func pixelWeight(x, y, w int) float64 {
	if PixelWeights == nil {
		return 1
	}
	if x < 0 || y < 0 {
		return 1
	}
	idx := y*w + x
	if idx < 0 || idx >= len(PixelWeights) {
		return 1
	}
	return PixelWeights[idx]
}

func computeColor(target, current *image.RGBA, lines []Scanline, alpha int) Color {
	var rsum, gsum, bsum, count int64
	a := 0x101 * 255 / alpha
	for _, line := range lines {
		i := target.PixOffset(line.X1, line.Y)
		for x := line.X1; x <= line.X2; x++ {
			tr := int(target.Pix[i])
			tg := int(target.Pix[i+1])
			tb := int(target.Pix[i+2])
			cr := int(current.Pix[i])
			cg := int(current.Pix[i+1])
			cb := int(current.Pix[i+2])
			i += 4
			rsum += int64((tr-cr)*a + cr*0x101)
			gsum += int64((tg-cg)*a + cg*0x101)
			bsum += int64((tb-cb)*a + cb*0x101)
			count++
		}
	}
	if count == 0 {
		return Color{}
	}
	r := clampInt(int(rsum/count)>>8, 0, 255)
	g := clampInt(int(gsum/count)>>8, 0, 255)
	b := clampInt(int(bsum/count)>>8, 0, 255)
	return Color{r, g, b, alpha}
}

func copyLines(dst, src *image.RGBA, lines []Scanline) {
	for _, line := range lines {
		a := dst.PixOffset(line.X1, line.Y)
		b := a + (line.X2-line.X1+1)*4
		copy(dst.Pix[a:b], src.Pix[a:b])
	}
}

func drawLines(im *image.RGBA, c Color, lines []Scanline) {
	const m = 0xffff
	sr, sg, sb, sa := c.NRGBA().RGBA()
	for _, line := range lines {
		ma := line.Alpha
		a := (m - sa*ma/m) * 0x101
		i := im.PixOffset(line.X1, line.Y)
		for x := line.X1; x <= line.X2; x++ {
			dr := uint32(im.Pix[i+0])
			dg := uint32(im.Pix[i+1])
			db := uint32(im.Pix[i+2])
			da := uint32(im.Pix[i+3])
			im.Pix[i+0] = uint8((dr*a + sr*ma) / m >> 8)
			im.Pix[i+1] = uint8((dg*a + sg*ma) / m >> 8)
			im.Pix[i+2] = uint8((db*a + sb*ma) / m >> 8)
			im.Pix[i+3] = uint8((da*a + sa*ma) / m >> 8)
			i += 4
		}
	}
}

func differenceFull(a, b *image.RGBA) float64 {
	size := a.Bounds().Size()
	w, h := size.X, size.Y
	var total float64
	for y := 0; y < h; y++ {
		i := a.PixOffset(0, y)
		for x := 0; x < w; x++ {
			ar := int(a.Pix[i])
			ag := int(a.Pix[i+1])
			ab := int(a.Pix[i+2])
			aa := int(a.Pix[i+3])
			br := int(b.Pix[i])
			bg := int(b.Pix[i+1])
			bb := int(b.Pix[i+2])
			ba := int(b.Pix[i+3])
			i += 4
			dr := ar - br
			dg := ag - bg
			db := ab - bb
			da := aa - ba
			err := float64(dr*dr + dg*dg + db*db + da*da)
			total += err * pixelWeight(x, y, w)
		}
	}
	denom := WeightSum
	if denom <= 0 {
		denom = float64(w * h)
	}
	return math.Sqrt(total/(denom*4)) / 255
}

func differencePartial(target, before, after *image.RGBA, score float64, lines []Scanline) float64 {
	size := target.Bounds().Size()
	w, h := size.X, size.Y
	denom := WeightSum
	if denom <= 0 {
		denom = float64(w * h)
	}
	total := math.Pow(score*255, 2) * denom * 4
	for _, line := range lines {
		i := target.PixOffset(line.X1, line.Y)
		for x := line.X1; x <= line.X2; x++ {
			tr := int(target.Pix[i])
			tg := int(target.Pix[i+1])
			tb := int(target.Pix[i+2])
			ta := int(target.Pix[i+3])
			br := int(before.Pix[i])
			bg := int(before.Pix[i+1])
			bb := int(before.Pix[i+2])
			ba := int(before.Pix[i+3])
			ar := int(after.Pix[i])
			ag := int(after.Pix[i+1])
			ab := int(after.Pix[i+2])
			aa := int(after.Pix[i+3])
			i += 4
			dr1 := tr - br
			dg1 := tg - bg
			db1 := tb - bb
			da1 := ta - ba
			dr2 := tr - ar
			dg2 := tg - ag
			db2 := tb - ab
			da2 := ta - aa
			wt := pixelWeight(x, line.Y, w)
			total -= float64(dr1*dr1+dg1*dg1+db1*db1+da1*da1) * wt
			total += float64(dr2*dr2+dg2*dg2+db2*db2+da2*da2) * wt
		}
	}
	if total < 0 {
		total = 0
	}
	return math.Sqrt(total/(denom*4)) / 255
}
